...
| `hos-gh-pull-request` | Write a pull request — a title carrying the linked issue's type emoji, and a body of `# Why` carrying the issue it closes, `# How` carrying the approach taken, and `# Note` where something has to be watched — shown in full first inside a fenced block, then opened with `gh pr create --draft` once you say so, and handed over as text alone where `gh` is missing or logged out. It states how the work was carried out, works out its base branch from the trunk marker above it, defaults the reviewer and the assignee, and covers the merge-only line a pull request that merges a trunk turns on. |
