...
| `hos-npm-publish-audit` | Audit a package before it reaches the npm registry, and report rather than release it — the artefact a consumer receives is read rather than the repository it was built from, and the inventory goes back in full. It covers the entry points resolved from outside the tree, the document followed the way a reader follows it, and the rules no linter enforces. Running the publish stays a person's. |
...
- **`hos-npm-publish-audit` reads the artefact, and stops before the registry.** Lint and tests examine the repository and neither opens the tarball a consumer receives, so the inventory is read and reported in full — a summary of it is exactly the second-hand statement the reading exists to stop trusting. Releasing stays a person's, because a version number is spent permanently whether or not the release is later withdrawn, and no reading afterwards undoes that. What got the tree into a state worth releasing — where the version bump sits among the commits — is the publishing convention's (`hoc-npm-publish`).
