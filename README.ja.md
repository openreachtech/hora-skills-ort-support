...
配布されるスキルは 3 件、すべて `support` ドメインのもので、内容は「コードそのものではなく、その周りの仕事」です — 書いていない人に結果を説明する、利用者が読むマニュアルを書く、決着した会話をスキルにする。各名前の先頭 3 文字がドメインを表すので、フラットに並んだスキル一覧を見た人が、どれがこのパッケージ由来かを一目で判別できます。ドメインごとに別のパッケージになっており、リポジトリは自分が扱うドメインのものを入れます。
